...
require( 'qc-support-promo-page/class-qc-support-promo-page.php' );

add_action( 'admin_init' , 'woo_tabbed_plugin_submenus' );
function woo_tabbed_plugin_submenus( $menu_ord ){
    global $submenu;

    $arr = array();

    $arr[] = isset($submenu['woo-tab'][0]) ? $submenu['woo-tab'][0] : '';
    $arr[] = isset($submenu['woo-tab'][301]) ? $submenu['woo-tab'][301] : '';
    $arr[] = isset($submenu['woo-tab'][300]) ? $submenu['woo-tab'][300] : '';
        ...
        <div id="message-tabbed" class="notice notice-info is-dismissible" style="padding:4px 0px 0px 4px;background:#C13825;">
            <?php
                printf(
                    ...
                    '<img src="'.esc_url(QC_WOO_TAB_IMAGE_URL).'/new-year-23.gif" >',
                    '</a>'
                );
