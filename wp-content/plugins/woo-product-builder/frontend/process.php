...
class VI_WPRODUCTBUILDER_F_FrontEnd_Process {
	...
	public function add_to_cart() {
		...
		if ( ! isset( $_POST['_nonce'] ) || ! isset( $_POST['woopb_id'] ) || ! $_POST['woopb_id'] ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}
		$post_id = sanitize_text_field(intval( $_POST['woopb_id'] ));// phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function remove_product() {
		$step_id = (int) sanitize_text_field($_GET['step'] ?? 0);
		$product_id = (int) sanitize_text_field($_GET['product_id'] ?? 0);

		if ( wp_verify_nonce( wp_unslash($_GET['_nonce'] ?? ''), '_woopb_remove_product_step' ) && $step_id && $product_id && get_post_type() == 'woo_product_builder' ) {
			$post_id = get_the_ID();
	...
	public function send_email_friend() {
		...
		$emailto = sanitize_email($_POST['woopb_emailto_field'] ??'');
		if ( ! $emailto ) {
			return;
		}
		$subject = sanitize_text_field($_POST['woopb_subject_field']??'');
		$content = sanitize_text_field($_POST['woopb_content_field']??'');
