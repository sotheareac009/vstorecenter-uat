...
class VI_WPRODUCTBUILDER_F_FrontEnd_Step {
	...
	public function step_title() {
		...
		$step_id    = (0 < $step_id && $step_id <= $count_tabs) ? $step_id : 1;
	...
	function add_vars( &$wp_query ) {
		...
		$page                                  = isset( $_GET['ppaged'] ) ? sanitize_text_field( $_GET['ppaged'] ) : '';
        ...
        <div class="woopb-steps">
			<?php if ( count( $step_titles ) ) {
					...
					<?php if ( count( $products_added ) ) { ?>
                        <div class="woopb-step woopb-step-<?php echo esc_attr( $k ) ?> <?php echo $current ? 'woopb-step-active' : ''; ?>">
                            <div class="woopb-step-products-added">
								<?php foreach ( $products_added as $p_id => $added_data ) { ?>
                                    <div class="woopb-step-products-added-wrapper">
										<?php
                                        $product = wc_get_product( $p_id );
										$quantity = floatval($quantity['quantity'] ?? $added_data);
										$sub_price     = floatval($product->get_price()) * $quantity;
                                        ...
                                        <div class="woopb-step-product-added">
											<?php echo '<span class="woopb-step-product-added-title">' . wp_kses_post( $product->get_name() ) . '</span> x ' . wp_kses_post( $quantity ) ?>
                                            <div class="woopb-step-product-added-price"><?php echo wc_price( $sub_price );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
											<?php
											$arg_remove = array( 'step' => ( $k + 1 ), 'product_id' => $p_id );
											?>
                                            <a class="woopb-step-product-added-remove"
                                               href="<?php echo wp_nonce_url( add_query_arg( $arg_remove ), '_woopb_remove_product_step', '_nonce' );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
                ...
                <div class="woopb-step woopb-step-total">
					<?php echo esc_html__( 'Total:', 'woo-product-builder' ) . ' ' . wc_price( $total_price );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                </div>
        ...
        <div class="woopb-mobile-control-bar">
            ...
            <div class="woopb-mobile-steps-control">
                ...
                <div class="woopb-mobile-view-total">
					<?php echo esc_html__( 'Total:', 'woocommerce-product-builder' ) . ' ' . wc_price( $total_price );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                </div>
	...
	public function init_scripts() {
		...
		if ( WP_DEBUG ) {
			...
			wp_enqueue_script( 'woo-product-builder', VI_WPRODUCTBUILDER_F_JS . 'woo-product-builder.js', array( 'jquery' ), VI_WPRODUCTBUILDER_F_VERSION, false );
		} else {
			...
			wp_enqueue_script( 'woo-product-builder', VI_WPRODUCTBUILDER_F_JS . 'woo-product-builder.min.js', array( 'jquery' ), VI_WPRODUCTBUILDER_F_VERSION, false );
		}
		if ( is_rtl() ) {
