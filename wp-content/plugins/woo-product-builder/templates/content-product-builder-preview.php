...
<div class="woocommerce-product-builder">
	<form method="POST" action="<?php echo esc_url( wc_get_cart_url() ) ?>" class="woopb-form">
				...
				foreach ( $products as $step_id => $items ) {
					foreach ( $items as $product_id => $quantity ) {
						...
						<tr>
							...
							<td>
								<a target="_blank" href="<?php echo esc_url( get_permalink( $product_id ) ); ?>" class="vi-chosen_title"><?php echo esc_html( $product_title ); ?></a> x <?php echo esc_html( $quantity ) ?>
							</td>
							<td><?php echo wp_kses_post($product->get_price_html() )?></td>

							<td class="woopb-total">
								<?php echo wc_price( ( $product_price * $quantity ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</td>
							<td>
								<?php
								$arg_remove = array(
								...
								?>
								<a class="woopb-step-product-added-remove" href="<?php echo wp_nonce_url( add_query_arg( $arg_remove ), '_woopb_remove_product_step', '_nonce' );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>"></a>
							</td>
						</tr>
						<?php
