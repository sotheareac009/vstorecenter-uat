...
class VI_WPRODUCTBUILDER_F_Widget_Price_Filter extends WC_Widget {
	...
	public function __construct() {
		...
		wp_register_script( 'accounting', WC()->plugin_url() . '/assets/js/accounting/accounting' . $suffix . '.js', array( 'jquery' ), WC_VERSION, false );
	...
	public function widget( $args, $instance ) {
		...
		$min_price = isset( $_GET['min_price'] ) ? sanitize_text_field( $_GET['min_price'] ) : apply_filters( 'woocommerce_price_filter_widget_min_amount', $min );// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$max_price = isset( $_GET['max_price'] ) ? sanitize_text_field( $_GET['max_price'] ) : apply_filters( 'woocommerce_price_filter_widget_max_amount', $max );// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$arg       = array(
		...
		echo '<form method="get" action="' . esc_url( $form_action ) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
		            ...
		            wc_query_string_form_fields( null, array( 'min_price', 'max_price' ), '', true ) . // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		            '<div class="clear"></div>
				</div>
			</div>
		</form>';
