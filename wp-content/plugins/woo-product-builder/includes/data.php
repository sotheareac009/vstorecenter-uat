	...
	public function get_product_filters( $post_id, $pagination = true ) {
		...
		if ( count( $item_data ) ) {
			...
			if ( is_array( $terms ) && count( $terms ) ) {
				...
				$where[]             = $wpdb->prepare( "tt2.term_id IN (%1s)", implode( ',', $terms ) );// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnquotedComplexPlaceholder
			...
			switch ( $sort_by ) {
				case 'price_low':
					...
					$product_args['meta_key'] = '_price';// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
					break;
				case 'price_high':
					$product_args['orderby']  = 'meta_value_num';
