...
if (!function_exists('themehunk_admin_menu')) {
  include_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
    ...
    add_action( 'admin_enqueue_scripts', 'admin_scripts');
     ...
     if ( !function_exists('themehunk_activeplugin') ) {
        function themehunk_activeplugin(){
           ...
           if (!wp_verify_nonce($_REQUEST['nonce'], 'thaps_admin_nonce')) {
             wp_die( - 1, 403 );
          }
    ...
    function themehunk_admin_menu(){
            ...
            add_menu_page(__('ThemeHunk', 'th-advance-product-search'), __('ThemeHunk', 'th-advance-product-search'), 'manage_options', 'themehunk-plugins', 'themehunk_plugins',  THEMEHUNK_PURL . '/th-option/assets/images/ico.png', 59);
        }
...
function admin_scripts( $hook ) {
    if ($hook === 'toplevel_page_themehunk-plugins'  ) {
      ...
      wp_localize_script(
          'themehunk-plugin-js', 'THAPSAdmin', array(
            'nonce'         => wp_create_nonce( 'thaps_admin_nonce' ),
          )
      );
    }
  }
