...
class Wf_Woocommerce_Packing_List_CustomizerLib {
	...
	public static function package_doc_items( $find_replace, $template_type, $order, $box_packing, $order_package ) {
		if ( ! is_null( $box_packing ) ) {
			...
			if ( Wf_Woocommerce_Packing_List::get_option( 'woocommerce_wf_packinglist_package_type' ) == 'box_packing' ) {
				...
				$find_replace['[wfte_box_name]'] = ( '' !== trim( $box_name ) ? esc_html( $box_name ) : '' );
			} else {
	...
	*/
	public static function set_other_data( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( false === $footer_data || '' === $footer_data ) {
	...
	public static function set_extra_charge_fields( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( ! is_null( $order ) ) {
			...
			//Subtotal ==========================
			if ( ! isset( $find_replace['[wfte_product_table_subtotal]'] ) ) {
				$incl_tax_text = '';
				$sub_total     = 0;
				$total_tax     = 0;
			...
			//total tax ==========================
			if ( ! isset( $find_replace['[wfte_product_table_total_tax]'] ) ) {
			...
			if ( '' !== $tax_items_row_html && isset( $tax_items_match[0] ) ) {
			...
			/**
			 * @since 4.3.0 - yith gift card compatibility
			 */

			$additional_row_arr = array();
			$order_totals_arr = $order->get_order_item_totals();

			if(!empty($order_totals_arr) && isset($order_totals_arr['gift_cards'])){
				$gf_price = Wt_Pklist_Common::get_yith_applied_coupon_total($order);
				$gf_code	= Wt_Pklist_Common::get_yith_applied_coupon_codes($order);
				$yith_gf_value = $gf_price > 0 ? Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $gf_price ) : '' ;
				if(!empty($yith_gf_value) && !empty($gf_code)){
					$yith_gf_value .= ' ('.implode(',',$gf_code).')';
				}
				$additional_row_arr['gift_cards'] = array(
					'label' => isset($order_totals_arr['gift_cards']['label']) ? $order_totals_arr['gift_cards']['label'] : '',
					'value' => $yith_gf_value,
				);
			}

			/**
			 * Fiter to add the additional row in the summary table.
			 *
			 * This filter allows modification of the additional row structure in the summary table.
			 *
			 * @since 4.3.0
			 * @param array  $additional_row_arr An associative array containing row data.
				 *                                   Format: array('some key' => array('label' => value, 'value' => value)).
				* @param string $template_type      The type of template being used.
				* @param array|object $order        The order data as an array or object.
				* @return array                     Modified additional row array.
				*/
			$additional_row_arr = apply_filters('wt_pklist_alter_additional_row_summary_table',$additional_row_arr,$template_type,$order);

			if(!empty($additional_row_arr)){
				$add_info_tr_matches	= array();
				$add_info_tr_html		= '';
				$this_st_row_key		= 'wfte_product_table_additional_info';
				if ( preg_match( '/<[^>]*data-row-type\s*=\s*"[^"]*\b' . $this_st_row_key . '\b[^"]*"[^>]*>(.*?)<\/tr>/s', $html, $add_info_tr_matches ) ) {
					$add_info_tr_html = $temp_tr_html = isset($add_info_tr_matches[0]) ? $add_info_tr_matches[0] : "";
					$tr_htm = '';
					if(!empty($add_info_tr_html)){
						foreach($additional_row_arr as $tr_key => $tr_value){
							if(!empty($tr_value) && isset($tr_value['label']) && !empty($tr_value['label']) && isset($tr_value['value']) && !empty($tr_value['value'])  ){
								$tr_htm_temp = str_replace('wfte_hidden','',$temp_tr_html);
								$tr_htm_temp = str_replace('[wfte_product_table_additional_info_label]',$tr_value['label'],$tr_htm_temp);
								$tr_htm_temp = str_replace('[wfte_product_table_additional_info_value]',$tr_value['value'],$tr_htm_temp);
								$tr_htm_temp = str_replace('additional_info',sanitize_title($tr_key),$tr_htm_temp);
								$tr_htm .= $tr_htm_temp;
							}
						}

						if(!empty($tr_htm)){
							$find_replace[$add_info_tr_html] = $tr_htm;
						}
					}
				}
			}
			...
			if ( ! empty( $fee_details ) ) {
				foreach ( $fee_details as $fee_detail ) {
					...
					$fee_detail_amount = (float) $order->get_line_total($fee_detail, false, true) + $fee_tax_amount;
			...
			if ( ! isset( $find_replace['[wfte_product_table_payment_total]'] ) || ! isset( $find_replace['[wfte_total_in_words]'] ) ) {
				...
				if ( ! empty( $refund_amount ) && 0 !== $refund_amount ) {
	...
	private static function generate_package_product_table_product_column_html( $wc_version, $the_options, $order, $template_type, $_product, $item, $columns_list_arr ) {
		...
		foreach ( $columns_list_arr as $columns_key => $columns_value ) {
			...
			if ( 'serial_no' === $columns_key || '-serial_no' === $columns_key ) {
				...
				if ( empty( $_product ) ) {
					$col_unit_price = $item_price;
				} else {
					$col_unit_price = apply_filters( 'wf_pklist_alter_package_item_price', $item_price, $template_type, $_product, $item, $order );
				}
	...
	*/
	private static function generate_product_image_column_data( $product_id, $variation_id, $parent_id, $img_style = array() ) {
		...
		if ( $product_id > 0 ) {
			...
			if ( empty( $attachment[0] ) && $variation_id > 0 ) {
	...
	*/
	private static function generate_product_table_product_row_html( $columns_list_arr, $template_type, $order = null, $column_list_options_value = array() ) {
		...
		if ( ! is_null( $order ) ) {
			...
			if ( version_compare($wc_version, '2.7.0', '<') ) {
				...
				$order_display_cart_ex_tax = Wt_Pklist_Common::get_order_meta( $order_id, '_display_cart_ex_tax', true );
			}
			$serial_no = 1;
	...
	public static function get_total_tax_column_display_option( $module_id, $template_type, $order ) {
		...
		if ( false === $total_tax_column_display_option ) {
	...
	public static function tax_column_in_product_table_row( $module_id, $template_type, $order, $order_item, $order_item_id, $_product = null ) {
		...
		if ( ! empty( $item_tax_subtotal ) ) {
			foreach ( $item_tax_subtotal as $tax_id => $tax_val ) {
				$tax_rate += ( isset( $tax_data_arr[ $tax_id ] ) && ! empty( $tax_val ) ? (float) $tax_data_arr[ $tax_id ] : 0 );
			}
		}
	...
	public static function generate_product_table_head_html( $columns_list_arr, $template_type ) {
		...
		foreach ( $columns_list_arr as $columns_key => $columns_value ) {
			...
			if ( 1 === $is_hidden || '1' === $is_hidden ) {
				...
				if ( $columns_value_updated == $columns_value ) {
		...
		if ( '' !== $end_td_key ) {
			...
			if ( $columns_class_added == $columns_list_arr[ $end_td_key ] ) {
	...
	*/
	public static function set_total_in_words( $total, $find_replace, $template_type, $html, $order = null ) {
		if ( false !== strpos( $html, '[wfte_total_in_words]' ) ) {
	...
	*/
	public static function set_total_weight( $find_replace, $template_type, $html, $order = null ) {
		$total_weight = 0;
		if ( false !== strpos( $html, '[wfte_weight]' ) ) {
			if ( ! is_null( $order ) ) {
				$order_items                   = $order->get_items();
				...
				if ( $order_items ) {
					...
					$weight_data = empty( $total_weight ) ? '' : $total_weight . ' ' . get_option( 'woocommerce_weight_unit' );
	...
	private static function get_default_order_fields( $key, $template_type, $order, $order_id, $wc_version ) {
		...
		if ( 'ssn' === $key ) {
			if ( ! $is_pro_customizer && 'invoice' !== $template_type && 'shippinglabel' !== $template_type ) {
				return '';
			}
		...
		if ( 'vat' === $key ) {
			...
			foreach ( $vat_fields as $vat_val ) {
				$res_vat = Wt_Pklist_Common::get_order_meta( $order_id, '_billing_' . $vat_val, true );
				if ( empty( $res_vat ) ) {
	...
	public static function set_extra_fields( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( ! is_null( $order ) ) {
			...
			if ( isset( $the_options[ 'wf_' . $template_type . '_contactno_email' ] ) && is_array( $the_options[ 'wf_' . $template_type . '_contactno_email' ] ) ) {
				...
				$ucf                 = 1;
				foreach ( $the_options[ 'wf_' . $template_type . '_contactno_email' ] as $val ) {
					if ( in_array( $val, $default_fields ) ) {
						$meta_vl = '';
						if ( empty( $meta_vl ) ) {
							$meta_vl = Wt_Pklist_Common::get_order_meta( $order_id, '_billing_' . $val, true );
							if ( empty( $meta_vl ) ) {
								$meta_vl = Wt_Pklist_Common::get_order_meta( $order_id, $val, true );
							}
						}
						...
						if ( $is_pro_customizer || ( ! $is_pro_customizer && 'invoice' === $template_type ) && $ucf === 1 ) {
							...
							if ( isset( $user_created_fields[ $val ] ) ) {
								...
								$meta_value = Wt_Pklist_Common::get_order_meta( $order_id, '_billing_' . $val, true );
								if ( ! $meta_value ) {
									$meta_value = Wt_Pklist_Common::get_order_meta( $order_id, $val, true );
									if ( ! $meta_value ) {
										$meta_value = Wt_Pklist_Common::get_order_meta( $order_id, '_' . $val, true );
										if ( ! $meta_value ) {
											$meta_value = '';
										}
									}
			...
			// default order meta fields - to show under extra fields - Backward compatibility
			$extra_fields['cus_note'] = self::get_default_order_fields( 'customer_note', $template_type, $order, $order_id, $wc_version );
			$extra_fields['vat']      = self::get_default_order_fields( 'vat', $template_type, $order, $order_id, $wc_version );
			$extra_fields['ssn']      = self::get_default_order_fields( 'ssn', $template_type, $order, $order_id, $wc_version );

			//shipping method
			$order_shipping = ( 0 === $wc_version ? $order->shipping_method : $order->get_shipping_method() );
			if ( Wt_Pklist_Common::get_order_meta( $order_id, '_tracking_provider', true ) || $order_shipping ) {
			...
			$tracking_details = Wt_Pklist_Common::get_order_meta( $order_id, ( '' !== trim( $tracking_key ) ? $tracking_key : '_tracking_number' ), true );
	...
	protected static function get_shipping_address( $template_type, $order = null ) {
		if ( ! is_null( $order ) ) {
			...
			if ( '' === trim( $shipping_address['first_name'] ) ) {
	...
	protected static function get_billing_address( $template_type, $order = null ) {
		if ( ! is_null( $order ) ) {
			if(!has_filter('wf_pklist_alter_billing_address')){
			...
			$billing_state        = Wt_Pklist_Common::get_order_meta( $order_id, '_billing_state', true );
			...
			if ( '' === trim( $billing_address['first_name'] ) ) {
	...
	public static function set_shipping_from_address( $find_replace, $template_type, $order = null ) {
		...
		$country_code = $country = isset( $country_arr[0] ) ? $country_arr[0] : '';
		...
		if(!has_filter('wf_pklist_alter_shipping_from_address')){ 
			$from_address_params = array(
				...
				'last_name'  => '',
				'company'    => '',
				'address_1'  => $the_options['woocommerce_wf_packinglist_sender_address_line1'],
				'address_2'  => $the_options['woocommerce_wf_packinglist_sender_address_line2'],
				'city'       => $the_options['woocommerce_wf_packinglist_sender_city'],
				'state'      => ( $the_options['woocommerce_wf_state_code_disable'] == 'yes' ? $state_full : $state ),
				'postcode'   => $the_options['woocommerce_wf_packinglist_sender_postalcode'],
	...
	*/
	...
	public static function hide_empty_elements( $find_replace, $html, $template_type ) {
		$hide_on_empty_fields = array(
			'wfte_vat_number',
			'wfte_ssn_number',
			'wfte_email',
			'wfte_tel',
			'wfte_shipping_method',
			'wfte_tracking_number',
			'wfte_footer',
			'wfte_return_policy',
	...
	*/
	public static function dummy_data_for_customize( $find_replace, $template_type, $html ) {
		...
		$find_replace['[wfte_customer_note]']                = __( 'Mauris dignissim neque ut sapien vulputate, eu semper tellus porttitor. Cras porta lectus id augue interdum egestas.', 'print-invoices-packing-slip-labels-for-woocommerce' );
