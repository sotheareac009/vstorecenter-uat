...
class Wf_Woocommerce_Packing_List_Shippinglabel
{
	...
	private function extra_fields_dummy_data($find_replace,$html,$template_type)
	{
		...
		$find_replace['[wfte_ship_date]']=gmdate('d-m-Y');
	...
	*/
	public function admin_settings_page()
	{
		...
		wp_enqueue_style('woocommerce_admin_styles',WC()->plugin_url().'/assets/css/admin.css', array(), WF_PKLIST_VERSION);
		wp_enqueue_media();
		if(!is_plugin_active('wt-woocommerce-shippinglabel-addon/wt-woocommerce-shippinglabel-addon.php') && isset($_GET['page']) && "wf_woocommerce_packing_list_shippinglabel" === $_GET['page']){ // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is a safe use of isset.
			wp_enqueue_script($this->module_id.'-pro-cta-banner',plugin_dir_url( __FILE__ ).'assets/js/pro-cta-banner.js',array('jquery'),WF_PKLIST_VERSION, false);
		}
		$params=array(
			'nonces' => array(
        ...
        if(!$is_pro_customizer)
        {
	    	...
	    	{   
		        ...
		        if(!is_null($this->customizer))
		        {
		        	...
		        	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Trusted HTML generated by shipping label module
		        	echo $html;
		        }
		        exit();
	    	}
	        ...
	        foreach ($orders as $order_id)
	        {
				...
				}else
				{
                    ...
                    if( 'No' === Wf_Woocommerce_Packing_List::get_option('woocommerce_wf_packinglist_preview') && 'print_shippinglabel' === $action ) {
                        ...
                        echo esc_html($no_item_error_message);
                        // Make sure to stop further execution.
                        exit();
                    } else {
                        wp_die(esc_html($no_item_error_message), "", array());
                    }
				}
