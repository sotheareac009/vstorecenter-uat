...
$inv_num=++$current_invoice_number;
    $date_frmt_tooltip=esc_html__('Click to append with existing data','print-invoices-packing-slip-labels-for-woocommerce');
...
<div id="invoice_number_prev_div" style="width: auto;border: 1px solid #dadadc;padding: 5px 12px;border-radius: 5px;display: inline-block;background: #f0f0f1;position: absolute;<?php echo is_rtl() ? "left: 1%;" : "right: 1%;"?>top: 0;">
    ...
    <p style="margin: 1em  0 0.5em 0; <?php if("No" === $use_wc_order_number){ echo "display: none;"; }?>" id="preview_invoice_number_text">
        <?php echo esc_html__('If the order number is','print-invoices-packing-slip-labels-for-woocommerce'); ?> <?php echo esc_html($order_number); ?>, 
        <br> 
        <?php echo sprintf(
            /* translators: %s: Document type (invoice, packing list, etc.) */
            esc_html__("the %s number would be",'print-invoices-packing-slip-labels-for-woocommerce'),esc_html($template_type)); ?> 
    </p>
    <p style="margin: 1em  0 0.5em 0; <?php if("Yes" === $use_wc_order_number){ echo "display: none;"; }?>" id="preview_invoice_number_text_custom">
        <?php echo sprintf(
            /* translators: %s: Document type (invoice, packing list, etc.) */
            esc_html__('Your next %s number would be','print-invoices-packing-slip-labels-for-woocommerce'),esc_html($template_type)); ?>
    </p>
