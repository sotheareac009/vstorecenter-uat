...
class SPFTESTIMONIAL {
	 ...
	 * @var string
	 */
	public static $version = SP_TFREE_VERSION;
	/**
	 * Dir.
	 *
	 * @var string
	...
	public static function add_admin_enqueue_scripts() {
		...
		if ( 'spt_testimonial_form' === $wpscreen->post_type || 'spt_shortcodes' === $wpscreen->post_type || 'spt_testimonial' === $wpscreen->post_type ) {
			...
			wp_enqueue_style( 'testimonials-navigation-icon', self::include_plugin_url( 'assets/css/fontello.css' ), array(), self::$version, 'all' );
		...
		if ( ! empty( $field_type ) ) {
			...
			if ( ! empty( $field['title'] ) ) {
				echo '<div class="spftestimonial-title">';
					...
					<span class="spftestimonial-help-text">' . $field['title_help'] . '</span>
