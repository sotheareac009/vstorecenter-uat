=== Shopire ===
Contributors: WPFable
Requires at least: 4.7
Tested up to: 6.9
Requires PHP: 5.6
Stable tag: 1.0.56
License: GPLv3 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.html
Tags: one-column, two-columns ,right-sidebar, flexible-header, custom-background, custom-header, custom-menu, editor-style, featured-images, footer-widgets, post-formats, theme-options, threaded-comments, translation-ready, full-width-template, custom-logo, blog, e-commerce, portfolio


== Description ==

Shopire is a fast, clean, lightweight theme. Shopire is customizable and modern-looking free responsive WooCommerce WordPress theme for e-commerce stores like clothing, fashion, electronics, ebooks, dress store, sports shop, marketplace, multivendor, equipments or online products. Theme is compatible with Elementor, WPML, Polylang, Yoast SEO, WooCommerce, Contact Form 7, Jetpack and other popular plugins. The theme is SEO friendly, WPML and RTL ready. Looking for a WooCommerce theme? Look no further! Shopire Pro demo https://demos.wpfable.com/premium/shopire/


== Installation ==
	
1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Shopire in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.


== Copyright ==

Shopire WordPress Theme, Copyright (c) 2025, WP Fable
Shopire is distributed under the terms of the GNU GPLs



== Credits ==

* Underscores - (C) 2012-2017 Automattic, Inc. - http://underscores.me/
License: GPLv2 or later (https://www.gnu.org/licenses/gpl-2.0.html) 

* Font Awesome 6.5.1 
Licenses - Font: SIL OFL 1.1, CSS: MIT License(http://fontawesome.io/license)
Source: https://github.com/FortAwesome/Font-Awesome

* Owl Carousel 2.3.4 - by @David Deutsch
Licenses - MIT License(https://github.com/OwlCarousel2/OwlCarousel2/blob/master/LICENSE)
Source: https://github.com/OwlCarousel2

* WOW - by Thomas Grainger
Licenses - MIT License (https://github.com/graingert/WOW/blob/master/LICENSE)
Source: https://wowjs.uk/

* Fancybox 
Licenses - GPLv3 License (https://github.com/fancyapps/fancybox)
Source: https://github.com/fancyapps/fancybox

* Animate Css by @Daniel Eden - http://daneden.me/animate
Licenses - MIT License(https://github.com/daneden/animate.css/blob/master/LICENSE)
Source: https://github.com/daneden/animate.css

* Customizer Controls 
Licenses - MIT license  (https://github.com/Codeinwp/customizer-controls/blob/master/LICENSE)
Source: https://github.com/Codeinwp/customizer-controls

* Customizer Repeater 
Licenses - MIT license (https://github.com/cristian-ungureanu/customizer-repeater/blob/production/LICENSE)
Source: https://github.com/cristian-ungureanu/customizer-repeater

* Range Controls 
Licenses - MIT license  (https://github.com/Codeinwp/customizer-controls/blob/master/LICENSE)
Source: https://github.com/Codeinwp/customizer-controls/tree/master/customizer-range-control

* WP Bootstrap Navwalker
Licenses - GNU GENERAL PUBLIC LICENSE (https://github.com/wp-bootstrap/wp-bootstrap-navwalker/blob/master/LICENSE.txt)
Source: https://github.com/wp-bootstrap/wp-bootstrap-navwalker

* Webfonts
Licenses - MIT license (https://opensource.org/license/MIT)
Source: https://github.com/WPTT/webfont-loader

* Sainitization
Licenses - GNU General Public License (https://github.com/WPTRT/code-examples/blob/master/LICENSE)
Source: https://github.com/WPTRT/code-examples

* Screenshot Image - https://stocksnap.io/photo/health-care-KQECHSBAOC , https://pxhere.com/en/photo/1114773 , https://pxhere.com/en/photo/627715 , https://pxhere.com/en/photo/1604655 , https://pxhere.com/en/photo/1631189 , https://pxhere.com/en/photo/645320 , https://stocksnap.io/photo/health-care-KQECHSBAOC
License: Creative Commons Zero, CC0 Public Domain


== Changelog ==

@version 1.0.56
* Style improvements - Mini Cart Widget Button

@version 1.0.55
* Tested - By WordPress 6.9

@version 1.0.54
* Style improvements - Footer Cart Widget Image

@version 1.0.53
* Style improvements - Footer Cart Widget Image

@version 1.0.52
* Style improvements - Footer Cart Widget Amount

@version 1.0.51
* Style improvements - Footer Cart Widget Checkout Button Hover & Focus

@version 1.0.50
* Style improvements - Footer Cart Widget Cart Button Hover & Focus

@version 1.0.49
* Style improvements - Footer Cart Widget Cart Button

@version 1.0.48
* Style improvements - Footer Product List

@version 1.0.47
* Style improvements - Footer Product List

@version 1.0.46
* Tested - By WordPress 6.8.3

@version 1.0.45
* Child Theme Code Added

@version 1.0.44
* Child Theme Code Added

@version 1.0.43
* Style improvements - Block Latest Comments

@version 1.0.42
* Style improvements - Block Latest Comments

@version 1.0.41
* Style improvements - Block Latest Comments

@version 1.0.40
* Style improvements - Block Outline Button

@version 1.0.39
* Style improvements - Block Button

@version 1.0.38
* Style improvements - Block Button

@version 1.0.37
* Style improvements - Block page List

@version 1.0.36
* Style improvements - Block Archives Hover & Focus Color

@version 1.0.35
* Style improvements - Block Archives Color

@version 1.0.34
* Style improvements - Block Archives

@version 1.0.33
* Tested - By WordPress 6.8.2

@version 1.0.32
* Style improvements - Block Archives

@version 1.0.31
* Style improvements - Block Archives

@version 1.0.30
* Style improvements - Block Preformatted

@version 1.0.29
* Style improvements - Block Verse

@version 1.0.28
* Style improvements - Block Details

@version 1.0.27
* Issue Added

@version 1.0.26
* Style improvements - Block List

@version 1.0.25
* Style improvements - WooCommerce Cart Page Product Title

@version 1.0.24
* Style improvements - Block List

@version 1.0.23
* Style improvements - Block Page List Hover & Focus

@version 1.0.22
* Style improvements - Product Title

@version 1.0.21
* Style improvements - Block Page List

@version 1.0.20
* Style improvements - Block Page List

@version 1.0.19
* Tested with WordPress 6.8.1

@version 1.0.18
* Style improvements - Block Page List

@version 1.0.17
* Style improvements - Product Button

@version 1.0.16
* Tested with WordPress 6.8

@version 1.0.15
* Style improvements - Product Button

@version 1.0.14
* Style improvements - Block Page List

@version 1.0.13
* Style improvements - Product Button

@version 1.0.12
* Style improvements - Product Title

@version 1.0.11
* Style improvements - Block Avatar

@version 1.0.10
* Style improvements - Block Previous Post

@version 1.0.9
* Style improvements - Block Next Post

@version 1.0.8
* Style improvements - Block Avatar

@version 1.0.7
* Style improvements - Block Previous Post

@version 1.0.6
* Style improvements - Block Next Post

@version 1.0.5
* Style improvements - WooCommerce Customer Account Icon

@version 1.0.4
* Style improvements - Block Read More

@version 1.0.3
* Style improvements - Block Archives

@version 1.0.2
* Style improvements - Compare Page

@version 1.0.1
* Child Theme Code Added

@version 1.0
* Style improvements - Block Verse

@version 0.9
* Style improvements - Block Details

@version 0.8
* Style improvements - Block Comments

@version 0.7
* Style improvements - Block Details

@version 0.6
* Upsale Links Added

@version 0.5
* Tested with WordPress 6.7.2

@version 0.4
* Theme & Author URI Added
* Upsale Links Added

@version 0.3
* Initial release