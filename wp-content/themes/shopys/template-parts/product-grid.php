...
function premium_product_grid_shortcode( $atts ) {
    $atts = shortcode_atts( array(
        ...
        'show_product_listing_header'   => 'true',   // show/hide table thead
    ), $atts, 'premium_products' );

    // Normalise: strip whitespace/newlines that multiline shortcode syntax can inject
...
// ── AJAX: paginate without page reload ────────────────────────────────────────
add_action( 'wp_ajax_ppg_ajax_paginate',        'ppg_ajax_paginate_handler' );
add_action( 'wp_ajax_nopriv_ppg_ajax_paginate', 'ppg_ajax_paginate_handler' );

function ppg_ajax_paginate_handler() {
